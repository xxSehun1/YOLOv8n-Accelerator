...
module PingPong_Ctrl (
    ...
    // =================================================================
    // FIX: 改用 Combinational Last 旗標精準轉態，消除 1-cycle overshoot
    // =================================================================
    logic filter_last, ifmap_last, ipsum_last, opsum_last;
    assign filter_last = filter_accept && (cnt_f_ch == Q*(R-1)) && (cnt_f_col == F_COL-1) && (cnt_f_row == F_ROW-1) && (cnt_f_num == P_T-1);
    assign ifmap_last  = ifmap_accept  && (cnt_i_ch == Q*(R-1)) && (cnt_i_row == E + F_ROW - 2) && (cnt_i_col >= F_COL - 1);
    assign ipsum_last  = ipsum_accept  && (cnt_p_ch == P_T-1) && (cnt_p_row == E-1);
    assign opsum_last  = opsum_accept  && (cnt_o_ch == P_T-1) && (cnt_o_row == E-1);
    ...
    always_comb begin
        ...
        case (state)
            ...
            S_FILTER:    if (filter_last)            next = S_IFMAP;
            S_IFMAP:     if (ifmap_last)             next = S_IPSUM;
            S_IPSUM:     if (ipsum_last)             next = S_OPSUM;
            S_OPSUM:     if (opsum_last)             next = (cnt_o_col == OFMAP_COL-1) ? S_START_OUT : S_IFMAP;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            if (state == S_IDLE && exec_valid && cfg_done && exec_op == 2'd0) begin
                ...
                shift    <= exec_shift; pconfig  <= exec_pconfig;
            end
                ...
                if (cnt_f_ch == Q*(R-1)) begin
                    ...
                    if (cnt_f_col == F_COL-1) begin
                        ...
                        if (cnt_f_row == F_ROW-1) begin
                            ...
                            if (cnt_f_num == P_T-1) cnt_f_num <= 0;
                            else                    cnt_f_num <= cnt_f_num + 1;
            ...
            if (opsum_accept) begin
                ...
                if (cnt_o_ch == P_T-1) begin
                    ...
                    if (cnt_o_row == E-1) begin
                        cnt_o_row <= 0;
                        if (cnt_o_col == OFMAP_COL-1) cnt_o_col <= 0;
                        else                          cnt_o_col <= cnt_o_col + 1;
    ...
    assign iob_out_len   = OFMAP_COL * E * P_T;
    ...
    assign oc_layer_last  = opsum_accept && (cnt_o_ch == P_T-1) && (cnt_o_row == E-1) && (cnt_o_col == OFMAP_COL-1);
